...
### Response variables
resp_vars <- c("NEE_gC_m2", "GPP_gC_m2", "Reco_gC_m2")  ## temp! ,  
...
for (t in 1:length(time)) {
 ...
 #vpd_terraclimate_sites <- as.data.frame(vpd_terraclimate_sites, xy=TRUE)
  ...
  file.remove(paste0("/home/master/predictors_1km/", "snowcover_", time[t], ".tif"))
  ...
  for (i in resp_vars) {
    ...
    mod2 = mod$finalModel #pull out the random forest object
    ...
    # parallel prediction with the random forest package
    library(itertools)
    # test parallel
    pred <-
      foreach(d=isplitRows(pred_rast_final, chunks=no_cores+2),
              .combine=c, .packages=c("randomForest")) %dopar% {
                predict(mod2, newdata=d, proximity = FALSE) }
    
    # # caret should also use parallel but for some reason these did not work - although on one day after a certain number of tests they did, but I wasn't able to get it to work the next day, very strange
    # library("caret")
    # library("randomForest")
    # library("foreach")
    # pred <- predict(object=mod, newdata=pred_rast_final, proximity = FALSE)  # use mod instead of mod2 and the parallel packages earlier
    # 
    # print("test other parallel")
    # pred <- predict(object=mod2, newdata=pred_rast_final, proximity = FALSE)  # use mod instead of mod2 and the parallel packages earlier
    # 
    # print("test third")
    # pred <- predict.train(object=mod, newdata=pred_rast_final, proximity = FALSE)  # use mod instead of mod2 and the parallel packages earlier
    # 
    ...
    rm(pred); rm(sub_pred); rm(pred_rast_final); rm(pred_matrix)
  ...
  # remove pred_rast
  rm(pred_rast)
  rm(pred_rasters) # moved from earlier
  gc()
  
  # files <- list.files("/home/master/temp/", full.names = T, pattern = "^file")
  # file.remove(files)
  # file.remove(files)
  # 
