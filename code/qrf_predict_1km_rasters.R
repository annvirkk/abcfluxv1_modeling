...
for (t in 1:length(time)) {
 ...
 #vpd_terraclimate_sites <- as.data.frame(vpd_terraclimate_sites, xy=TRUE)
  ...
  fake_rast[[1]][fake_rast[[1]] < 100000] <- 21  # esa has to have some values!
  gc()
  ...
  # pred_rasters[[1]] <- round(pred_rasters[[1]], digits=0) # had to round esa too?
  
  # say that we have a categorical rasters - this is important, otherwise error!
  pred_rasters[[1]] <- as.factor(pred_rasters[[1]])
  pred_rasters[[16]] <- as.factor(pred_rasters[[16]])
  ...
  for (i in resp_vars) {
    ...
    # # simple model
    # print(paste0("/home/master/abcfluxv1_modeling/results/", paste(i,  "1km_qrf", "train_loocv_simple",sep="_"), ".rds"))
    # # Load model files
    # mod <- readRDS(paste0("/home/master/abcfluxv1_modeling/results/", paste(i, "1km_qrf",  "train_loocv_simple",  sep="_"), ".rds"))
    # mod2 = mod$finalModel #pull out the quantile regression object
    
    # subset the region to 4 domains
    c(-5e+06, 0, -4.6e+06, 1e+06) # east can
    c(-5e+06, 0, 1e+06, 4.2e+06) # west can and alaska - note there will be overlap
    c(0, 5e+06, -4.6e+06, 1e+06) # west eurasia - argh, this crashes
    c(0, 5e+06, 1e+06, 4.2e+06) # east eurasia - this crashes for sure
    # eli siis varmaan joku 6 aluetta tarvitaan
    

    
    pred_rasters2 <- crop(pred_rasters, ext(0, 5e+06, -4.6e+06, 1e+06))
    
    # now with full extent
    pred <- predict(model=mod2, object=pred_rasters,  na.rm=TRUE, proximity = FALSE, filename="/home/master/temp/tempr.tif", overwrite=TRUE,  cores=8, cpkgs=c("randomForest")) # kun laitto tämän tuli error what=c(0.025, 0.5, 0.975)
    # 
    ...
    # 
    # # data frame
    #pred <- predict(object=mod2, newdata=pred_data[, 3:19], proximity = FALSE) # changing to 6000000 produces this error: Error in predict.randomForest(object, newdata = newdata, nodes = TRUE) : 
    ...
    # pred <- predict(object=mod2, newdata=pred_data[, 3:13], what=c(0.5), proximity = FALSE)
    ...
    # try with mod instead
    pred <- predict(model=mod, object=pred_rasters,  na.rm=TRUE, proximity = FALSE, filename="/home/master/temp/tempr.tif", overwrite=TRUE,  cores=15, cpkgs=c("randomForest", "quantregForest")) 
    # 
